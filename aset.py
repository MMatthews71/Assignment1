...
class ASet(Set[T]):
    ...
    def __and__(self, other):
        res = ASet(min(len(self), len(other)))  # O(n)
        for i in range(len(self)):              # O(n)
    ...
    def __or__(self, other):
        maxcapacity = len(self.array) + len(other.array)    # O(1)
        res = ASet(maxcapacity)                             # O(n+m)
        for the_set in [self, other]:                       # O(1)
            for i in range(len(the_set)):                   # O(n+m)
                res.add(the_set.array[i])                   # O(n+m)
        return res
